...
module src_manager
  (
   ...
   assign src = opr_rdy ? opr :
		~kill_spec1 & (exdst1 == opr[`RRF_SEL-1:0]) ? exrslt1 :
		~kill_spec2 & (exdst2 == opr[`RRF_SEL-1:0]) ? exrslt2 :
		~kill_spec3 & (exdst3 == opr[`RRF_SEL-1:0]) ? exrslt3 :
		~kill_spec4 & (exdst4 == opr[`RRF_SEL-1:0]) ? exrslt4 :
		~kill_spec5 & (exdst5 == opr[`RRF_SEL-1:0]) ? exrslt5 : opr;

   assign resolved = opr_rdy |
		     (~kill_spec1 & (exdst1 == opr[`RRF_SEL-1:0])) |
		     (~kill_spec2 & (exdst2 == opr[`RRF_SEL-1:0])) |
		     (~kill_spec3 & (exdst3 == opr[`RRF_SEL-1:0])) |
		     (~kill_spec4 & (exdst4 == opr[`RRF_SEL-1:0])) |
		     (~kill_spec5 & (exdst5 == opr[`RRF_SEL-1:0]));
